...
@Component
public class SpainPrivateFlowProvider implements FlowDefinitionProvider {

    @Override
    public FlowDefinition define() {
        return new FlowDefinition(Country.SPAIN, CustomerType.PRIVATE, List.of(
                StepDefinition.checkStep("IDENTITY_VERIFICATION", "Identity verification",
                        "Collect DNI/NIE and run a Clave/DNIe-style document verification check.",
                        List.of(
                                FieldDefinition.text("fullName", "Full legal name", true),
                                FieldDefinition.text("idNumber", "DNI / NIE", true,
                                        "^[XYZxyz]?\\d{7,8}[A-Za-z]$", "Use DNI (8 digits + letter) or NIE (X/Y/Z + 7-8 digits + letter).",
                ...
                StepDefinition.dataStep("CONTACT_DETAILS", "Contact details",
                        ...
                        List.of(
                                ...
                                FieldDefinition.text("postalCode", "Postal code", true, "^\\d{5}$", "Use 5 digits.", null),
                ...
                StepDefinition.dataStep("FINANCIAL_PROFILE", "Financial profile",
                        "Employment, income, housing costs and dependants.",
                        List.of(
                                FieldDefinition.select("employmentStatus", "Employment status", true,
                                        List.of("EMPLOYED", "SELF_EMPLOYED", "UNEMPLOYED", "RETIRED", "STUDENT")),
                                FieldDefinition.number("monthlyIncome", "Monthly net income (EUR)", true),
                                FieldDefinition.number("monthlyDebtPayments", "Monthly debt payments (EUR)", true),
                                FieldDefinition.number("housingCost", "Monthly housing cost (EUR)", true),
